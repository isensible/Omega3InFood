            ...
            string displayName = "")
        {
            ...
            if (formFile.ContentType.ToLower() != "text/plain")
            {
                modelState.AddModelError(formFile.Name, 
                                         $"The {displayName} ({fileName}) must be a text file.");
            }
            ...
            {
                modelState.AddModelError(formFile.Name, $"The {displayName} ({fileName}) is empty.");
            }
            else if (formFile.Length > 1048576)
            {
                modelState.AddModelError(formFile.Name, $"The {displayName} ({fileName}) exceeds 1 MB.");
            }
            else
            {
                try
                {
                                ...
                                detectEncodingFromByteOrderMarks: true))
                    {
                        ...
                        {
                            modelState.AddModelError(formFile.Name, 
                                                     $"The {displayName} ({fileName}) is empty.");
                        }
                    }
                }
                catch (Exception ex)
                {
                    modelState.AddModelError(formFile.Name, 
                                             $"The {displayName} ({fileName}) upload failed. " +
